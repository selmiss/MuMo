...
class GeoGraphLyaerMPNN(torch.nn.Module):
    def __init__(
        ...
        self.first_graph_embedder = (node_dim != hidden_channels) or (
        ...
        if self.brics:
            ...
            if fra_edge_index.shape[1] != 0:
                ...
                if self.first_graph_embedder:
                    ...
                    fra_edge_attr = F.relu(
                        self.lin_b(fra_edge_attr)
                    )  # (N, 3) -> (N, hidden_channels)
                    if geo_operation:

                        bafra_edge_attr = F.relu(
                            self.lin_c(bafra_edge_attr)
                        )  # (N, 1) -> (N, hidden_channels)

                if geo_operation:
                    hd_fra_edge_attr = F.relu(
                        self.geo_conv(fra_edge_attr, bafra_edge_index, bafra_edge_attr)
                    )
                    beta = self.geo_gate(
                        torch.cat(
                            [
                                fra_edge_attr,
                                hd_fra_edge_attr,
                                fra_edge_attr - hd_fra_edge_attr,
                            ],
                            1,
                        )
                    ).sigmoid()
                ...
                fra_hidden_states = F.relu(
                    self.graph_conv(fra_x, fra_edge_index, fra_edge_attr)
                )
                beta = self.lin_gate(
                    torch.cat([fra_x, fra_hidden_states, fra_x - fra_hidden_states], 1)
                ).sigmoid()
