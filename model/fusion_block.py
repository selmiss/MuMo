...
class HierarchicalFusionBlock_without_self_attention(nn.Module):
    def __init__(self, config, layer_idx, global_inject=False, num_graph_layers=2, graph_enabled=True, brics=True, geo_operation=True):
        super().__init__()
        ...
        self.first_graph_layer = GeoGraphLyaerMPNN(9, 3, config.hidden_size, brics=brics, geo_operation=geo_operation)
        self.graph_layers = torch.nn.ModuleList(
            [GeoGraphLyaerMPNN(config.hidden_size, config.hidden_size, config.hidden_size, brics=brics, geo_operation=geo_operation) for i in range(self.num_graph_layers)]
        )
        ...
        if self.global_inject:
            self.graph_map_layer = nn.Linear(config.hidden_size + self.graph_hidden_size, config.hidden_size)
    ...
    def forward(
        ...
        if self.graph_enabled:
            graph_hidden_states, graph_attention_mask = graph2batch_sequence(graph_hidden_states=graph_data.x, batch=graph_data.batch)
            ...
            graph_data.x = sequence2batch_graph(graph_hidden_states, graph_attention_mask)
...
class TransformerFusionBlock(nn.Module):
    ...
    def forward(
        ...
        # Main stream pass the self-attention
        residual = hidden_states
        attention_output = self.attention(hidden_states=self.attn_ln(hidden_states), attention_mask=attention_mask)
        hidden_states = attention_output[0] + residual
