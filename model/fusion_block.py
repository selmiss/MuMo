...
class HierarchicalFusionBlock(nn.Module):
    def __init__(self, config, layer_idx, global_inject=False, num_graph_layers=2, graph_enabled=True):
        super().__init__()
        ...
        self.first_graph_layer = GeoGraphLyaerMPNN(9, 3, config.hidden_size, geo_operation=True)
        self.graph_layers = torch.nn.ModuleList(
            [GeoGraphLyaerMPNN(config.hidden_size, config.hidden_size, config.hidden_size, geo_operation=True) for i in range(self.num_graph_layers)]
        )
