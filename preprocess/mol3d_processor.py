...
def check_3d_data_source(input_path):
    ...
    for item in tqdm(data):
        item = {key: convert_value(key, value) if isinstance(value, list) else value for key, value in item.items()}
...
def check_geo_data(data: Data, type_map: dict):
    ...
    for key, value in data.items():
        if key in type_map and key != "smiles" and value.dtype != type_map[key]:
            print(f"Type error: key '{key}' type is {value.dtype}, expect {type_map[key]}")
            mark = False
    
    # Check basic shape.
    try:
        if not (
            data.x.shape[1] == 9 and
            data.x.shape[0] > 0 and
            data.edge_attr.shape[1] == 3 and
            data.ba_edge_attr.shape[1] == 1 and
            data.fra_edge_attr.shape[1] == 3 and
            data.cluster_idx.shape[0] == data.x.shape[0] and
            data.bafra_edge_attr.shape[1] == 1
        ):
    ...
    if not (
        ...
        data.ba_edge_index.shape[0] == 2 and
        data.fra_edge_index.shape[0] == 2 and
        data.bafra_edge_index.shape[0] == 2
    ):
    ...
    if torch.max(data.ba_edge_index) > data.edge_attr.shape[0] - 1:
        print(f"'ba_edge_index' out of range, with max index {torch.max(data.ba_edge_index)}")
        mark = False
    if torch.max(data.fra_edge_index) > data.x.shape[0] - 1:
        print(f"'fra_edge_index' out of range, with max index {torch.max(data.fra_edge_index)}")
        mark = False
    if torch.max(data.bafra_edge_index) > data.fra_edge_attr.shape[0] - 1:
        print(f"'bafra_edge_index' out of range, with max index {torch.max(data.bafra_edge_index)}")
        mark = False
    return mark
...
def check_batch(data_batch):
    ...
    """
    # Check if edge indices and batch arrays are integers
    if not (data_batch.edge_index.dtype == torch.int64 and 
            data_batch.ba_edge_index.dtype == torch.int64 and
            data_batch.fra_edge_index.dtype == torch.int64 and
            data_batch.bafra_edge_index.dtype == torch.int64 and
            data_batch.batch.dtype == torch.int64):
    ...
    if num_fra_edges > num_ab_edges:
        print(f"Error: Processed atom-bond edges ({num_fra_edges}) exceed original atom-bond edges ({num_ab_edges}).")
        return False
    ...
    if len(data_batch.smiles) != num_graphs:
        print(f"Error: Number of smiles ({len(data_batch.smiles)}) does not match number of graphs ({num_graphs}).")
        return False
        ...
        for filename in filenames:
            if filename.endswith('.csv'):
                try:
                    ...
                    process_dataset_and_save_multithreaded(input_file, output_file, num_workers=10)
                except Exception as err:
                ...
                try:
                    ...
                    with open(jsonl_path, 'r', encoding='utf-8') as file:
                        for line in file:
                            ...
                            filtered_obj = {key: obj[key] for key in type_map.keys() if key in obj}
...
if __name__ == "__main__":
    ...
    # test_data_dir = "your_data_dir/dataset/test_randomsplit/bace"
    # test_data_output_dir = "your_data_dir/dataset/test_geo_randomsplit/bace"
    # generate_geotest_dataset_dir(test_data_dir, test_data_output_dir)
    
    # test_data_dir = "your_data_dir/dataset/test_scaffoldsplit"
    # test_data_output_dir = "your_data_dir/dataset/test_geo_scaffoldsplit"
    # generate_geotest_dataset_dir(test_data_dir, test_data_output_dir)
    
    # input_path = "your_data_dir/dataset/test_geo/sider_1/raw/train_sider_1.csv"
    # check_3d_data_source(input_path)
    
    # output_file = "your_data_dir/dataset/pretrain/geo_data/test_merge.jsonl"
