...
def parse_args():
    ...
    parser.add_argument(
        "--world_size", type=int, default=1, help="Total number of GPUs"
    )
    # Output arguments
    parser.add_argument(
...
def main():
    ...
    for i in tqdm(
        range(0, len(test_dataset), args.batch_size * args.world_size),
        total=(len(test_dataset) + args.batch_size * args.world_size - 1)
        ...
        batch_df = pd.DataFrame(
            {"smiles": smiles_list_this_round, "prediction": predictions.flatten()}
        )
