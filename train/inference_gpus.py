...
def process_batch_for_inference(batch, tokenizer, max_length):
    ...
    def tokenize_and_batch(inputs):
        ...
        tokenized = tokenizer(
            smiles, truncation=True, max_length=max_length,
    ...
    def batch_graph_data(inputs):
        def convert_value(key, value):
            try:
                ...
                print(f"Error converting key '{key}' with value type {type(value)}: {str(e)}")
                raise e
...
def main():
    ...
    if not os.path.exists(tmp_results_file):
        pd.DataFrame(columns=["smiles", "prediction"]).to_csv(tmp_results_file, index=False)
    ...
    if args.scaler_path:
        ...
        predictions = all_predictions.flatten()  # Make sure predictions are 1-dimensional
