...
def process_dataset(raw_datasets, target_column_name):
    import pdb
    ...
    for item in raw_datasets:
        ...
        for key in graph_uni.keys:
            if key == "batch":
                continue
            data[key] = graph_uni[key]

        item["smiles"] = smiles
        item["graph"] = data
        item[target_column_name] = labels
    return raw_datasets
...
def main():
    ...
    if True:
        ...
        def process_dataset_map(example):
            labels = example[target_column_name]
            smiles = " ".join([mol["smiles"] for mol in example["mol_list"]])
            ...
            batch_data = [
                ...
                for mol_dict in example["mol_list"]
            ]
            graph_uni = TriBatch.from_data_list(batch_data)
            ...
            example["graph"] = data
            example[target_column_name] = labels
            return example

        raw_datasets = raw_datasets.map(process_dataset_map, batched=True)
    ...
    num_input_smiles = config.num_input_smiles
    ...
    def custom_collate_fn(batch):
        ...
        graph_batch = TriBatch.from_data_list(batch["graph"])
    ...
    trainer.model.floating_point_ops = lambda x: 0
