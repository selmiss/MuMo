...
def parse_arguments():
    ...
    parser.add_argument('--model_class', type=str, help='Your model path', required=True)
    parser.add_argument('--data_type', type=str, help='Your model path', required=True)
    parser.add_argument('--tasks', type=lambda x: x.split(','), help='Your tasks, divided by comma. ', required=True)
...
def main():
    ...
    BASE_DIR = "xxx"
    root_dir = "DATA_DIR/model/sft/" + args.model_name
    ...
    for task in args.tasks:
        ...
        if args.data_type != "None":
            model_dir_name = model_name + "_" + args.model_class + '_' + args.data_type + '-' + task
        else:
        ...
        results_dir = os.path.join(BASE_DIR, "results", model_name + "_" + args.model_class)
        ...
        print(BASE_DIR + "/results" + model_dir_name + ".csv")

    combined_df = pd.concat([pd.read_csv(csv_file) for csv_file in csv_files], axis=1)

    final_csv_path = os.path.join(BASE_DIR, "results", model_name + "_" + args.model_class, "combined_results.csv")
